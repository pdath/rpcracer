...
config_load(const char *path)
{
    ...
    /* reconnect_delay_ms (required, must be positive) */
    if (json_uint32(yyjson_obj_get(root, "reconnect_delay_ms"),
                    &cfg->reconnect_delay_ms, "reconnect_delay_ms") != 0)
        goto fail;

    /* stall_threshold_ms (required, must be positive) */
    if (json_uint32(yyjson_obj_get(root, "stall_threshold_ms"),
